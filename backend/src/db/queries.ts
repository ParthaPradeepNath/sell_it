...
export const getAllProducts = async () => {
  return db.query.products.findMany({
    with: { user: true }, // this will not only user_id but also all the user details thanks to the relations
    orderBy: (products, { desc }) => [desc(products.createdAt)], // desc means: you will see the latest products first
    // the square brackets are required becase Drizzle ORM's orderBy expects an array, even for a single column.
  });
};
